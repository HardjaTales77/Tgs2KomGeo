...
public class Calculation {
    ...
    /**
     * asumsi nilai kordinat xMin, yMin, xMax, dan yMax dimiliki oleh titik yang
     * berbeda
     *
    ...
    /**
     * Melakukan incremental Sweeping pada array of point
     *
     * @param p kumpulan titik
     * @return kumpulan titik yang sudah menjadi convex hull yang telah terurut
     * secara counter-clockwise
     */
    public Point[] incrementaSweeping(Point[] p) {
        ArrayList<Point> l = new ArrayList<>();// untuk menampung convex hull saat ini
        if (p.length <= 4) {//kalau jumlah titik yang diterima kurang atau sama dengan 4 titik
            return p;//langsung dikembalikan, karena sudah pasti convex hull
        } else {//kalau jumlah titik yang diterima lebih dari 4
            l.add(p[0]);//masukan titik pertama yang berada di ujung paling kiri berdasarkan nilai x nya
            int bawah, atas, nbawah, natas;//index-index untuk mencari titik yang bersebelahan
            //bawah=index yang dituju oleh titik baru pada bagian atas convex hull sementara
            //nbawah=index berikutnya setelah index yang dituju oleh titik baru pada bagian atas convex hull sementara
            //atas=index yang dituju oleh titik baru pada bagian bawah convex hull sementara
            //natas=index berikutnya setelah index yang dituju oleh titik baru pada bagian bawah convex hull sementara
            boolean isSAtas = false;//status segitiga yang menjadi convex hull awalnya, apakah segitiga atas atau bawah
            //isSAtas bernilai true bila convex hull adalah segitiga yang menghadap ke atas
            if (p[0].y > p[1].y) {//bila convex hull adalah segitiga bawah
                nbawah = 1;//titik ke-1 menjadi nbawah
                natas = 0;//titik ke-0 menjadi natas
            } else {//segitiga atas
                nbawah = 0;//titik ke-0 menjadi nbawah
                natas = 1;//titik ke-1 menjadi natas
                isSAtas = true;//mengubah value isAtas mejadi true karena ini adalah segitiga atas
            }
            int kanan = 2;//index titik paling kanan dari convex hull saat ini, yaitu 2
            bawah = kanan;//index penghubung ke bagian bawah convex hull diinisialisasi menjadi index titik yang paling kanan
            atas = kanan;//index penghubung ke bagian atas convex hull diinisialisasi menjadi index titik yang paling kanan
            for (int i = 3; i < p.length; i++) {//untuk setiap titik dimulai dari titik ke-4 pada array titik
                if (i == 3) {//apabila index=3(titik ke-4), maka dibuat agar segitiga jadi ccw di ArrayList
                    if (!isSAtas) {//kalau bukan segitiga atas
                        l.add(p[2]);//memasukan titik ke-2
                        l.add(p[1]);//memasukan titik ke-3
                    } else {//kalau segitiga bawah
                        l.add(p[1]);//memasukan titik ke-2
                        l.add(p[2]);//memasukan titik ke-3
                    }
                }
                //mengecek ke bagian bawah convex hull
                while (this.ccw(p[i], l.get(bawah), l.get(nbawah)) < 0) { //selama masih belok kiri
                    bawah = nbawah;//mengupdate index titik yang dicek menjadi nbawah
                    if (nbawah >= 0) {//kalau titik nbawah masih lebih dari 0 (karena index tidak mungkin negatif)
                        nbawah--;//melanjutkan dengan mengurangi index
                    }
                }
                //hasil: p[i]-->bawah-->nbawah
                //mengecek bagian atas convex hull
                while (this.ccw(p[i], l.get(atas), l.get(natas)) > 0) { //selama masih belok kanan
                    atas = natas;//mengupdate index titik yang dicek menajdi natas
                    if (i == 3) {//kalau titik ke-4 (index ke-3)
                        natas--;//titik natas melanjutkan ke index yang lebih rendah
                    } else {//kalau bukan titik ke-4 (bukan index ke-3)
                        natas++;//titik natas melanjutkan ke index yang lebih tinggi
                    }
                }
                if (i > 3) {//kalau titik yang dicek setelah titik ke-4 (index ke-3)
                    //buang titik di dalam polygon baru
                    for (int j = bawah + 1; j < atas; j++) {//melakukan iterasi untuk seluruh titik yang berada 
                        //di antar index bawah+1 sampai index atas
                        l.remove(j);//membuang titik pada convex hull baru
                    }
                }
                this.addMiddle(l, p[i], bawah + 1);//menyisipkan titik baru pada posisi index ke-bawah+1 
            }
            return p;//mengeluarkan hasil incremental sweeping berupa array off Point
        }
    }

    /**
     * Metode ini memasukkan sebuah titik ke dalam arraylist (di sela-sela titik
     * lain)
     *
    ...
    public ArrayList<Point> addMiddle(ArrayList<Point> l, Point p, int index) {
        ArrayList<Point> hasil = new ArrayList<>();//hasil dari penambahan titik di tengah ArrayList
        for (int i = 0; i < l.size(); i++) {//untuk semua titik pada array list l
            if (i == index) {//cek apakah sekarang i=masukan index
                hasil.add(p);//ditambahkan titik p
                hasil.add(l.get(i));//ditambahkan titik ke-i pada l
            } else {//jika i bukan index
                hasil.add(l.get(i));//ditambahkan titik ke-i pada l
            }
        }
        return hasil;//kembalikan hasil arraylist yang sudah ditambah titik
    }

    /**
     * Metode ini memanggil metode lain untuk mencari pasangan titik dengan
     * jarak terjauh
     *
     ...
     *
     * @param p kumpulan titik
     * @return String yang berupa index dari kedua titik yang memiliki jarak
     * terjauh
     */
    public String idxLongestPair(Point[] p) {
        ...
        int aS = 0;//variabel string yang menyimpan index titik pertama
        int bS = 0;//variabel string yang menyimpan index titik kedua
        ...
        //mengembalikan String index dari kedua titik pembentuk longest pair
        if (aS < bS) {//kalau aS<bS
            return aS + " " + bS;//kembalikan aS dan bS dalam bentuk String
        } else {//kalau bS<aS
            return bS + " " + aS;//kembalikan bS dan aS dalam bentuk String
        }
    }
    ...
    /**
     * Metode ini menggunakan algoritma Rotating Calipers untuk mencari 2 titik
    ...
    /**
     * Metode bruteforce untuk mencari jarak terpendek pada index yang sudah
     * ditentukan
     *
     * @param p kumpulan titik
     ...
     *
     * @param s kumpulan titik yang berada di dalam jangkauan jarak sebelumnya
     * dari garis pembagi
    ...
    /**
     * Metode rekursif devide and conquer untuk mencari titik dengan jarak
     * terdekat
     *
     * @param p kumpulan titik
     * @param q kumpulan titik
     * @param start index dimulai
     * @param n index akhir param p dan q merupakan array titik yang sama
     * @return garis yang terbentuk dari dua pasangan titik terdekat
     */
    ...
    public String idxClosestPair(Point[] p) {
        ...
        //mengembalikan String index dari kedua titik pembentuk closest pair
        if (aS < bS) {//kalau aS<bS
            return aS + " " + bS;//kembalikan aS dan bS dalam bentuk String
        } else {//kalau bS<aS
            return bS + " " + aS;//kembalikan bS dan aS dalam bentuk String
